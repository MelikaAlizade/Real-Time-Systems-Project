...
class RMScheduler:
    ...
    def generate_tasks(self, num_tasks, target_utilization=None):
        ...
        periods = [
            ...
            80,
            100,
        ]  # RM works best with harmonic periods
    ...
    def analyze_results(self, tasks, max_time, scenario_name):
        ...
        for task in tasks:
            ...
            if self.is_preemptive:
                theoretical_response = self._calculate_response_time_preemptive_rm(
                    task, core_tasks
                )
            else:
                theoretical_response = self._calculate_response_time_nonpreemptive_rm(
                    task, core_tasks
                )
    ...
    def _calculate_response_time_preemptive_rm(self, task, core_tasks):
        ...
        hp_tasks = sorted(
            [t for t in core_tasks if t.period < task.period], key=lambda x: x.period
        )
    ...
    def _calculate_response_time_nonpreemptive_rm(self, task, core_tasks):
        ...
        sum_hp = sum(
            (t.wcet + (t.wcet - t.net)) for t in sorted_tasks if t.period < task.period
        )
