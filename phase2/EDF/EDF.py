...
class Scheduler:
    ...
    def _simulate_core_edf(self, core, max_time):
        ...
        while time < max_time and (jobs or ready_queue or current_job):
            ...
            if current_job:
                ...
                execute_until = min(
                    time + execution, current_job.deadline, next_release
                )
    ...
    def analyze_results(self, tasks, max_time, scenario_name):
        ...
        for task in tasks:
            ...
            if self.is_preemptive:
                theoretical_response = calculate_response_time_preemptive(
                    task, core_tasks
                )
            else:
                theoretical_response = calculate_response_time_nonpreemptive(
                    task, core_tasks
                )
...
def calculate_response_time_preemptive(task, core_tasks):
...
def calculate_response_time_nonpreemptive(task, core_tasks):
    ...
    J_i = (
        max((t.wcet + (t.wcet - t.net)) for t in core_tasks if t.period > task.period)
        if any(t.period > task.period for t in core_tasks)
        else 0
    )
    ...
    sum_hp = sum(
        (t.wcet + (t.wcet - t.net)) for t in core_tasks if t.period < task.period
    )
...
def run_comprehensive_scenarios():
    ...
    for policy in ["partitioned_best_fit", "global"]:
        scenarios.append(
            {
                "name": f"{policy}_EDF_50tasks_8cores",
                "num_cores": 8,
                "num_tasks": 50,
                "scheduling_policy": "EDF",
    ...
    for policy in ["partitioned_best_fit", "global"]:
        scenarios.append(
            {
                "name": f"{policy}_non_preemptive_EDF_50tasks_8cores",
                "num_cores": 4,
                "num_tasks": 100,
                "scheduling_policy": "EDF",
                "allocation_policy": policy,
    ...
    for util in [0.25, 0.5, 0.75]:
        for alloc_policy in ["partitioned_best_fit", "partitioned_worst_fit"]:
            scenarios.append(
                {
                    "name": f"{alloc_policy}_EDF_100tasks_4cores_util{util}",
                    "num_cores": 4,
                    "num_tasks": 100,
                    "scheduling_policy": "EDF",
                    "allocation_policy": alloc_policy,
